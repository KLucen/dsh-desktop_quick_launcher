...
function renderProbeFunctions() {
	return [
		...
		"",
		"function Get-Probe {",
		"  $result = @{ probeClass = 'down'; status = 0; instanceId = ''; authRequired = $false }",
...
function apply(ctx, config, hooks) {
	...
	*/
	...
	const defaultSpawnSurvivor = async (file) => {
		...
		const taskName = `DSH-Web-Restart-${portFromUrl(current().url ?? "http://127.0.0.1:3080")}`;
		const created = await runCommand("schtasks", [
			...
			taskName,
			"/tr",
			`powershell.exe ${powershellArgs.map((part) => part.includes(" ") ? `"${part}"` : part).join(" ")}`,
			"/sc",
			"once",
			"/st",
			"00:00"
	...
	(async () => {
		...
		await runCommand("schtasks", [
			...
			"/f",
			"/tn",
			`DSH-Web-Restart-${portFromUrl(current().url ?? "http://127.0.0.1:3080")}`
		]);
	})();
}
//#endregion
export { Config, LAUNCHER_API, NONCE_HEADER, PLUGIN_VERSION, apply, createDesktopShortcut, findOpenTurns, formatDuration, inject, isLauncherFailure, name, parseStatusFile, phaseSeverity, portFromUrl, renderLauncherScript, renderRestartHelper, resolveLauncherSpec, stripBom, tailLines };
